...
verify=config.get('naverSiteVerification','').strip()
verify_meta=f'<meta name="naver-site-verification" content="{html.escape(verify)}">' if verify else '<!-- 네이버 서치어드바이저 HTML 태그 인증값을 site.config.json의 naverSiteVerification에 입력하세요. -->'
...
print('Generated index.html, sitemap.xml, robots.txt, rss.xml')
